...
namespace Champerof.Controllers
{
    ...
    public class MenuController : ControllerBase
    {
        ...
        public MenuController(IMenuRepository menuRepository, ValidationService validation)
        {
            ...
            string searchValue = "")
        {
            try
            {
                var data = await _menuRepository.GetAllMenu(
        ...
        public async Task<IActionResult> Add(Menu menu)
        {
            try
            {
                var Name = _validation.ValidateRequired(menu.Name, "Name"); 
                if (!Name.IsSuccess) { return Ok(Name); }

                var (IsSuccess, Message, Id, Extra) = await _menuRepository.AddOrUpdateMenu(menu);
        ...
        public async Task<IActionResult> GetById(long id)
        {
            try
            {
                var data = await _menuRepository.GetMenuById(id);
        ...
        public async Task<IActionResult> Delete(long id)
        {
            try
            {
                var (IsSuccess, Message, Id, Extra) = await _menuRepository.DeleteByMenuId(id);
                CommonViewModel.IsSuccess = IsSuccess;
                CommonViewModel.IsConfirm = true;
                CommonViewModel.StatusCode = IsSuccess ? ResponseStatusCode.Success : ResponseStatusCode.Error;
                CommonViewModel.Message = Message;
