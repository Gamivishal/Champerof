...
namespace Champerof.ServiceRepository.PaymentFollowUpRepository
{
    public class PaymentFollowUpRepository : IPaymentFollowUpRepository
    {
        ...
        // pending
        public async Task<PagedResult<PaymentFollowUp>> GetAllpending(
 int start,
 int length,
 string sortColumn,
 string sortColumnDir,
 ...
 long invoiceId)
        ...
        public async Task<PaymentFollowUp?> GetByIdpending(long id)
        {
            var parameters = new[]
            {
                new SqlParameter("@Id", id),
            ...
            var result = _repositoryBase.ExecuteSingle("SP_PendingPaymentFollowUp_Get", parameters);

            return await Task.FromResult(result);
        }

        public async Task<(bool IsSuccess, string Message, long Id, List<string> Extra)> AddOrUpdatePending(PaymentFollowUp model)
        {
            List<SqlParameter> oParams = new()
            {
                new SqlParameter("@Id", model.Id),
            ...
            var result = _repositoryBase.ExecuteStoredProcedurenew("SP_PendingPaymentFollowUp_Save", oParams, true);

            return await Task.FromResult(result);
        }

        public async Task<(bool IsSuccess, string Message, long Id, List<string> Extra)> DeletePending(long id)
        {
            List<SqlParameter> parameters = new()
            {
                new SqlParameter("@Id", id),
            ...
            var result = _repositoryBase.ExecuteStoredProcedurenew("SP_PendingPaymentFollowUp_Delete", parameters, true);
        ...
        //history
        public async Task<PagedResult<PaymentFollowUp>> GetAllHistory(
int start,
int length,
string sortColumn,
string sortColumnDir,
...
long FollowupId)
        ...
        new SqlParameter("@FollowupId", FollowupId)
            ...
            var result = _repositoryBase.ExecuteWithPagination(
                "SP_PendingPaymentFollowUp_History_Get",
                parameters,
                start,
                length,
                sortColumn,
